...
import { usesSessionResume } from '../agent/capability';
...
export async function startRunFlow(input: StartRunFlowInput): Promise<StartRunFlowResult> {
  ...
  if (!resumeFrom && usesSessionResume(input.capability.agentId)) {
...
export function recordRunSessionEvent(input: RecordRunSessionEventInput): void {
  ...
  if (usesSessionResume(input.capability.agentId) && input.event.sessionId) {
